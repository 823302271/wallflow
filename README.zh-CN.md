...
在全屏或 Space 切换过程中，Wallflow 始终保留同一个桌面窗口、WebKit 表面和渲染器，并保持固定的桌面层级。桌面不可见时只暂停动画时钟、媒体、音频和输入；桌面重新可见时从同一帧继续。macOS 系统静态桌面只用于 Wallflow 没有运行时的兜底，不再参与 Space 切换。连接或断开其他显示器时，仍连接显示器上的渲染器会被复用，不再重新启动壁纸。
