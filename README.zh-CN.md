...
在全屏或 Space 切换过程中，Wallflow 只暂停现有渲染器，让同一个窗口保持在最后一帧；桌面重新可见后再继续播放，不会创建第二个播放器，也不会重新加载壁纸。Wallflow 不修改系统壁纸，因为 macOS 会按 Space 分别保存桌面图片。连接或断开其他显示器时，仍连接显示器上的渲染器会被复用，不再重新启动壁纸。
