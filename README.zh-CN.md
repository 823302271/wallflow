...
- 导入成功后会自动加入 **壁纸管理...**；已有的 Wallflow 受管导入目录也会在启动时自动发现。
- 移除受管 ZIP 壁纸时，确认后会删除 Wallflow 安装的副本；外部文件或 URL 只会移除库记录，不会删除原始文件。
...
Wallflow 仅面向 Apple 芯片，并以 Metal 作为统一渲染主干。程序避免持续进行 CPU 侧模拟，会在空闲时降低帧率、以低于 Retina 原生分辨率的比例渲染、使用双缓冲交换链，并暂停桌面被应用窗口联合遮住的显示器。网页壁纸限制为 24 FPS 和 1 倍设备像素比，并且只有选择网页项目时才启动 WebKit。原生视频通过 AVFoundation 播放，使用短缓冲和 1080p 解码上限。

默认开启 **桌面不可见时暂停**：Wallflow 会计算所有可见应用窗口的联合覆盖区域，只有某块显示器已经没有桌面区域暴露时，才停止对应的渲染、媒体、音频和壁纸输入。这也能识别由多个窗口共同组成全屏画面的浏览器。普通应用窗口不会导致暂停，只要仍有桌面区域暴露，动画就继续运行。应用窗口上的点击会被过滤，只有暴露桌面区域中的点击才会传给壁纸。

在全屏或 Space 切换过程中，Wallflow 只暂停现有渲染器，让同一个窗口保持在最后一帧；桌面重新可见后再继续播放，不会创建第二个播放器，也不会重新加载壁纸。由于 WindowServer 在 Space 过渡中可能先绘制系统桌面、再合成第三方窗口，Wallflow 会在每个桌面 Space 被访问后，把当前壁纸的一张静态帧同时设置为 macOS 系统桌面兜底。连接或断开其他显示器时，仍连接显示器上的渲染器会被复用，不再重新启动壁纸。
...
第一条命令验证项目加载、壁纸库、桌面可见性规则、包边界检查、场景解析、LZ4、内嵌图片、DXT 和场景图层构建。第二条命令会启动真实的 WKWebView，验证 Wallpaper Engine 属性和鼠标 API。第三条命令会打开真实视频，验证播放、暂停/恢复和静态帧抓取。第四条命令会把中英文壁纸管理窗口渲染为 PNG 测试图。
