...
- 壁纸窗口加入每个桌面 Space，切换桌面后继续实时播放
- 视频暂停写入播放检查点，恢复时按检查点 seek 续播；暂停会话由宿主在稳定播放后提交
- 暂停时原位冻结当前帧；系统桌面兜底图避开 Space 切换过程更新
- 菜单栏暂停和继续控制
...
- 本地文件、项目目录和 ZIP 导入后都会复制到 `~/Library/Application Support/Wallflow/ImportedWallpapers`，不依赖原始路径或临时目录；旧版本留下的外部本地路径会自动迁移到受管目录，已有的 Wallflow 受管导入目录也会在启动时自动发现。
