...
- 壁纸窗口不加入全部 Space（避免 1→2→3→桌面 连切时在中间桌面误恢复导致跳帧）
- 视频暂停写入播放检查点，恢复时按检查点 seek 续播；暂停会话由宿主在稳定播放后提交
- 暂停时截取当前帧并设为该屏系统桌面，退出 Wallflow 后仍可见
- 菜单栏暂停和继续控制
...
**暂停与恢复（检查点模型）：** 桌面不可见或手动暂停时，Wallflow 会把当前
媒体时间写入内存检查点，并截取该帧作为冻结画面与系统桌面兜底；恢复时始终
根据检查点 `seek` 到该时间再播放，而不是依赖播放器在长暂停期间保持 playhead。
因此切到其他 Space停留较久再回到桌面，也会从暂停点丝滑续播，而不会跳到未来帧。
...
- 合成 DOM 鼠标移动、指针移动和左键点击事件；右键留给系统桌面菜单，不拦截
- 媒体自动播放和远程网页内容访问
- 音频与媒体监听器注册兼容层
...
在全屏或 Space 切换过程中，Wallflow 始终保留同一个桌面窗口、WebKit 表面和渲染器，并固定在系统壁纸之上、桌面图标之下。可见性按**每台显示器独立**计算：一块屏切 Space或被全屏覆盖时，只暂停该屏，不会拖累其他屏幕。桌面不可见前会保存最后一帧并写入系统桌面兜底；返回桌面时先显示冻结帧，视频等再从检查点 seek 就绪后揭开实时画面，避免跳帧或露出白色空表面。连接或断开其他显示器时，仍连接显示器上的渲染器会被复用，不再重新启动壁纸。鼠标仅桥接左键到壁纸；右键留给系统桌面菜单，不被壁纸层拦截。
